...
项目直接以仓库根目录作为构建根目录。发布与更新命令见 [GitHub 发布说明](PUBLISH.md)，旧 WPF 版本保留在 Git 历史中。项目采用 [MIT 许可证](LICENSE)，第三方依赖许可见 [第三方声明](THIRD_PARTY_NOTICES.md)。

## 构建

从旧目录布局迁移后，已有 CMake 构建缓存中的绝对路径会失效。请在仓库根目录重新执行下方配置和构建命令，生成新的 `out/release` 构建目录；旧 `out/windows*` 目录保留的产物可按需取用。
